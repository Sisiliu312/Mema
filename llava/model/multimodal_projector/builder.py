...
class SpatialGate(nn.Module):
    ...
    def forward(self, x, c_l):
        ...
        # print("h max:", h.max().item(), "min:", h.min().item(), "mean:", h.mean().item())
        # print("delta max:", delta.max().item(), "min:", delta.min().item(), "mean:", delta.mean().item())
        # print("a max:", a.max().item(), "min:", a.min().item(), "mean:", a.mean().item())
        return x + a * delta
...
class DynamicSharingUnit(nn.Module):
    ...
    def forward(self, y_l, text_global, c_prev):
        ...
        i_logit = self.Wi(s) + self.bi
        f_logit = self.Wf(s) + self.bf
        # print("i_logit:", i_logit.abs().mean().item(), i_logit.abs().max().item())
        # print("f_logit:", f_logit.abs().mean().item(), f_logit.abs().max().item())\
        
        # print("i meas+std:", i.mean().item(), i.std().item())
        # print("f meas+std", f.mean().item(), f.std().item())
...
class TextConditionedDynamicLayerAttention(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        nn.init.constant_(self.g.delta[0].weight, 0.0)
        nn.init.constant_(self.g.delta[0].bias,   0.0)

        nn.init.constant_(self.g.delta[2].weight, 0.0)
        nn.init.constant_(self.g.delta[2].bias,   0.0)
    ...
    def forward(self, text_features, projected_layer_features, force_off: bool = False):
        ...
        print("output max:", output.max().item(), "min:", output.min().item(), "mean:", output.mean().item())
