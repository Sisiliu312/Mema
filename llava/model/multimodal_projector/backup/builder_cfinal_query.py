...
class TextConditionedDynamicLayerAttention(nn.Module):
    ...
    def __init__(self, feature_dim=4096, num_vision_layers=24, 
                 num_heads=8, reduction_ratio=4, M=32):
        super().__init__()
        ...
        self.slots = nn.Parameter(torch.randn(M, feature_dim) * 0.02)

        # condition from c_final
        self.cond = nn.Linear(feature_dim, feature_dim, bias=True)
    ...
    def forward(self, text_features, projected_layer_features, force_off: bool = False):
        ...
        # --- build KV tokens from RAW patch tokens (no refresh) ---
        # [L,N,D] -> [LN,D]
        # kv_tokens = torch.cat(projected_layer_features, dim=0)  # [L*N, D]
        kv_tokens = projected_layer_features[-2]
        LN = kv_tokens.shape[0]

        # K/V projections + LN (in D space), then reshape to heads
        K = self.k_norm(self.W_k(kv_tokens))  # [LN, D]
        V = kv_tokens

        # --- build Q from learnable slots conditioned by c_final ---
        # [M,D] + [M,D]
        q = self.slots + self.cond(c_final).unsqueeze(0)         # [M, D]
        q = self.q_norm(self.W_q(q))                              # [M, D]

        # reshape to SDPA format: [B=1, H, Len, Hd]
        q = q.view(self.M, self.num_heads, self.head_dim).transpose(0, 1).unsqueeze(0)     # [1,H,M,Hd]
        k = K.view(LN, self.num_heads, self.head_dim).transpose(0, 1).unsqueeze(0)         # [1,H,LN,Hd]
        v = V.view(LN, self.num_heads, self.head_dim).transpose(0, 1).unsqueeze(0)         # [1,H,LN,Hd]

        attn = F.scaled_dot_product_attention(q, k, v, attn_mask=None, dropout_p=0.0, is_causal=False)
        # [1,H,M,Hd] -> [M,D]
        attn = attn.squeeze(0).transpose(1, 2).contiguous()   # [H,M,Hd]
        attn = attn.transpose(0, 1).contiguous().view(self.M, self.feature_dim)

        out = self.output_norm(self.W_o(attn))  # [M,D]
