...
class TextConditionedDynamicLayerAttention(nn.Module):
    ...
    def forward(self, text_features, projected_layer_features, force_off: bool = False):
        ...
        evidence_list = []
        score_list = []
        layer_id_list = []   # 记录每个 token 来自哪一层（用于 focus 保底）
        ...
        all_layer_ids = torch.cat(layer_id_list, dim=0)     # [K_total]
        ...
        # 你可以把这两个做成 self.final_k / self.focus_min
        final_k = min(64, evidence_tokens.shape[0])         # ✅ 最终输出 token 数（建议 64 或 80）
        focus_min = min(final_k // 2, evidence_tokens.shape[0])  # ✅ focus 层保底（可调）

        # 1) focus 保底：从 focus 层 token 中先取 focus_min 个
        ...
        # 3) 可选：保持最终 token 仍按分数从高到低排序（更稳定）
        ...
        evidence_tokens = evidence_tokens.index_select(0, selected_idx)  # [final_k, D]
        # print("selected evidence_tokens shape:", evidence_tokens.shape)
        ...
        # groups = self.group_proj(groups)  # 可先注释掉试试
