...
class TextConditionedDynamicLayerAttention(nn.Module):
    ...
    def forward(self, text_features, projected_layer_features, force_off: bool = False):
        ...
        evidence_list = []
        score_list = []
        layer_id_list = []   # 仍然保留，但下面不会再用它做保底
        ...
        evidence_tokens = torch.cat(evidence_list, dim=0)   # [K_total, D]  候选池
        all_scores = torch.cat(score_list, dim=0)           # [K_total]
        # all_layer_ids = torch.cat(layer_id_list, dim=0)   # 可留着debug，但不参与选择

        final_k = min(64, evidence_tokens.shape[0])

        # ===== 全局 z-score（在候选池内做）=====
        scores_f = all_scores.float()
        mean = scores_f.mean()
        std = scores_f.std(unbiased=False) + 1e-6
        scores_z = (scores_f - mean) / std   # [K_total]

        # 用 z-score 选 top64
        top_z, top_idx = torch.topk(scores_z, k=final_k, largest=True, sorted=True)
        evidence_tokens = evidence_tokens.index_select(0, top_idx)
