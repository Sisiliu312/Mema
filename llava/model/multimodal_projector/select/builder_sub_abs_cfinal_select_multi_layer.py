...
class DynamicSharingUnit(nn.Module):
    ...
    def forward(self, y_l, text_global, c_prev):
        ...
        # base: 纯绝对量
        base = torch.cat([c_prev_norm, y_l, text_global], dim=-1)  # 3D

        # diff: 差分
        diff = (c_prev_norm - y_l)

        # 方式A：把 diff 先线性投影到 dim，再以小系数加到 y_l 或 s 上
        diff_proj = self.Wd(diff)          # Wd: Linear(dim, dim) or Linear(dim, reduced_dim)
        y_l2 = y_l + 0.1 * diff_proj       # 0.05~0.2 之间扫一下

        combined = torch.cat([c_prev_norm, y_l2, text_global], dim=-1)
