...
class TextConditionedDynamicLayerAttention(nn.Module):
    ...
    def forward(self, text_features, projected_layer_features, force_off: bool = False):
        ...
        for li, patches in enumerate(projected_layer_features):
            ...
            # confidence: 峰值 z-score（越大越“这层确实有证据”）
            top1 = scores.max()
            conf = top1  # 因为已z-score化，top1本身就是“峰值强度”
            layer_conf.append(conf)
        ...
        # focus 层保底（你可以调，比如 total_k 的 1/2 或固定32）
        focus_min = total_k // 2
        k_budget = total_k - focus_min

        # 先给 focus 层拿 focus_min
        k_per_layer = [0] * L
        k_per_layer[focus] = min(focus_min, layer_scores[focus].numel())

        # 对其他层：只给“高置信层”分配
        # 阈值可以先用 2.0（z-score意义：峰值超过均值2个标准差）
        thresh = 3.0
        other_idx = [i for i in range(L) if i != focus and layer_conf[i].item() > thresh]
        # print("other_idx:", other_idx)

        if len(other_idx) > 0 and k_budget > 0:
            # 按置信度软分配预算
            conf_vals = layer_conf[other_idx]
            weights = torch.softmax(conf_vals, dim=0)  # [M]
            alloc = torch.floor(weights * k_budget).long()

            # 处理整数舍入导致的剩余
            rem = int(k_budget - alloc.sum().item())
            if rem > 0:
                # 把剩余给权重最大的层
            ...
            # 每层至少给1（否则 floor 可能全0）
            for j, li in enumerate(other_idx):
        ...
        for li in range(L):
            ...
            evidence_list.append(patches.index_select(0, top_idx))

        evidence_tokens = torch.cat(evidence_list, dim=0) if len(evidence_list) else torch.empty((0, D), device=text_features.device, dtype=text_features.dtype)
