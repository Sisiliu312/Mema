...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, low_cpu_mem_usage=True, **kwargs):
    ...
    if 'llava' in model_name.lower():
        ...
        if model_base is not None:
            ...
            from peft import PeftModel
            tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
            model = AutoModelForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=low_cpu_mem_usage, **kwargs)
    ...
    if 'llava' in model_name.lower():
        ...
        if device_map is not None and device_map != "auto":
            device = device_map.get("", "cuda:0") if isinstance(device_map, dict) else device_map
